...
void log_test_mode_plan(void) {
    if (script_count == 0) {
        test_mode_log(LOG_INFO, "[test-mode] No configured scripts; nothing would run");
        return;
    }
    ...
    test_mode_log(LOG_INFO, "[test-mode] Would execute %d script section(s) in this order:", script_count);
    for (int i = 0; i < script_count; ) {
        ...
        int group_size = j - i;
        test_mode_log(LOG_INFO, "[test-mode] parallel-group priority=%u count=%d", cur_prio, group_size);
        ...
        for (int k = i; k < j; k++) {
            ...
            if (e->critical && e->simulate_exit_code != 0) {
                test_mode_log(LOG_ERR, "[test-mode] critical script '%s' simulate_exit_code=%d: FAILED", e->name, e->simulate_exit_code);
                critical_fail = true;
            }
        }
        ...
        if (critical_fail) {
            ...
            for (int ni = j; ni < script_count; ) {
                ...
                remaining++;
                while (ni < script_count && scripts[order[ni]].priority == np)
                    ni++;
            }
            test_mode_log(LOG_ERR, "[test-mode] critical failure in priority group %u -- aborting %d remaining group(s); inhibitor would release", cur_prio, remaining);
            return;
        }
