...
static bool run_systemctl_reboot(void)
{
    pid_t pid = fork();
    if (pid < 0) {
        syslog(LOG_ERR, "Failed to fork for reboot command: %s",
              strerror(errno));
        return false;
    }
    ...
    if (wret < 0) {
        syslog(LOG_ERR, "Failed waiting for reboot command: %s",
              strerror(errno));
        return false;
    }
...
static bool run_systemctl_poweroff(void)
{
    pid_t pid = fork();
    if (pid < 0) {
        syslog(LOG_ERR, "Failed to fork for poweroff command: %s",
              strerror(errno));
        return false;
    }
    ...
    if (wret < 0) {
        syslog(LOG_ERR, "Failed waiting for poweroff command: %s",
              strerror(errno));
        return false;
    }
...
static bool reload_runtime_configuration(const char *selected_config_path_runtime,
                     bool selected_config_is_custom)
{
    if (control_test_mode) {
        load_selected_config(selected_config_is_custom ?
                     selected_config_path_runtime : NULL);
        return true;
    }
...
static bool status_append(char **buf, size_t *len, size_t *cap,
              const char *fmt, ...)
{
    va_list ap;
    va_list ap_copy;
...
static void handle_control_client_request(int client_fd,
                      ...
                      bool selected_config_is_custom)
{
    ...
    if (strcmp(cmd, "RELOAD") == 0) {
        if (reload_runtime_configuration(selected_config_path_runtime,
                         selected_config_is_custom))
    ...
    if (strcmp(cmd, "GUARD") == 0) {
        ...
        if (strcmp(arg, "ENABLE") == 0) {
            if (control_test_mode) {
                write_control_response(client_fd, true,
                          "guard enable simulated (test mode)");
                return;
            }
            bool ok = guard_runtime_set_enabled(true);
            write_control_response(client_fd, ok,
                          ok ? "guard enabled" : "guard enable failed");
            return;
        }
        ...
        if (strcmp(arg, "DISABLE") == 0) {
            ...
            write_control_response(client_fd, ok,
                          ok ? "guard disabled" : "guard disable failed");
            return;
        }
    ...
    if (strcmp(cmd, "SHUTDOWN") == 0) {
        char *arg = strtok_r(NULL, " \t", &saveptr);
        if (!arg) {
            write_control_response(client_fd, false,
        ...
        if (strcmp(arg, "DISABLE") == 0) {
            ...
            write_control_response(client_fd, ok,
                          ok ? "shutdown disabled" : "shutdown disable failed");
            return;
        }
        ...
        if (strcmp(arg, "ENABLE") == 0) {
            ...
            write_control_response(client_fd, ok,
                          ok ? "shutdown enabled" : "shutdown enable failed");
            return;
        }
    ...
    if (strcmp(cmd, "REBOOT") == 0) {
        ...
        if (control_test_mode) {
            write_control_response(client_fd, true,
                          skip ? "reboot simulated (skip-scripts, test mode)"
                               : "reboot simulated (test mode)");
            return;
        }

        if (!guard_runtime_unmask_forced()) {
            write_control_response(client_fd, false,
                          "failed to unmask shutdown targets");
            return;
        }
        ...
        write_control_response(client_fd, true,
                      skip ? "reboot requested (skip-scripts)"
                           : "reboot requested");
        return;
    }
    ...
    if (strcmp(cmd, "POWEROFF") == 0) {
        ...
        if (control_test_mode) {
            write_control_response(client_fd, true,
                          skip ? "shutdown simulated (skip-scripts, test mode)"
        ...
        write_control_response(client_fd, true,
                      skip ? "shutdown requested (skip-scripts)"
                           : "shutdown requested");
        return;
    }
    ...
    if (strcmp(cmd, "SET_LOGIND_INHIBITOR_DELAY") == 0) {
        ...
        if (extra) {
            write_control_response(client_fd, false,
                          "unexpected SET_LOGIND_INHIBITOR_DELAY arguments");
            return;
        }
        ...
        if (current_logind_inhibit_delay != max_inhibit_delay) {
            write_control_response(client_fd, false,
                          "failed to set logind inhibit delay");
            return;
        }

        snprintf(msg, sizeof(msg),
             "logind inhibit delay set to %d", max_inhibit_delay);
...
int control_setup_socket(void)
{
    ...
    if (fd < 0) {
        syslog(LOG_ERR, "Failed to create control socket: %s",
              strerror(errno));
        return -1;
    }
    ...
    if (bind(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        syslog(LOG_ERR, "Failed to bind control socket %s: %s",
    ...
    if (chmod(socket_path, S_IRUSR | S_IWUSR) < 0) {
        syslog(LOG_ERR, "Failed to chmod control socket %s: %s",
              socket_path, strerror(errno));
        close(fd);
        (void)unlink(socket_path);
        return -1;
    ...
    if (listen(fd, 8) < 0) {
        syslog(LOG_ERR, "Failed to listen on control socket %s: %s",
              socket_path, strerror(errno));
        close(fd);
        (void)unlink(socket_path);
        return -1;
                 ...
                 bool selected_config_is_custom)
{
    ...
    if (client_fd < 0) {
        if (errno != EINTR && errno != EAGAIN && errno != EWOULDBLOCK)
            syslog(LOG_ERR, "accept() on control socket failed: %s",
                  strerror(errno));
        return;
    }

    handle_control_client_request(client_fd,
                  selected_config_path_runtime,
                  selected_config_is_custom);
    close(client_fd);
}
