                      ...
                      sd_bus_error *ret_error)
{
    ...
    if (r < 0) {
        syslog(LOG_ERR, "Failed to parse PrepareForShutdown signal: %s",
              strerror(-r));
        return r;
    }
    ...
    if (control_consume_skip_shutdown_scripts_once()) {
        syslog(LOG_NOTICE,
              "Skipping script/priority execution for this shutdown due to runtime reboot --skip-scripts request");
        if (inhibitor_fd >= 0) {
            close(inhibitor_fd);
            inhibitor_fd = -1;
        }
    ...
    if (inhibitor_fd >= 0) {
        syslog(LOG_INFO,
              "All scripts finished. Releasing inhibitor lock.");
        close(inhibitor_fd);
        inhibitor_fd = -1;
    }
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "c:tf", long_options,
                   &option_index)) != -1) {
        switch (opt) {
        case 'c':
            config_path = optarg;
        ...
        default:
            fprintf(stderr,
                "Usage: %s [-c PATH] [--config PATH] [--test-mode] [--foreground]\n",
                argv[0]);
            return EXIT_FAILURE;
        }
    }
    ...
    if (config_path && config_path[0] != '/') {
        ...
        if (!getcwd(cwd, sizeof(cwd))) {
            fprintf(stderr,
                "terminusd: getcwd() failed: %s\n",
                strerror(errno));
            return EXIT_FAILURE;
        }

        int n = snprintf(config_path_buf, sizeof(config_path_buf),
                 "%s/%s", cwd, config_path);
        if (n < 0 || (size_t)n >= sizeof(config_path_buf)) {
            fprintf(stderr,
                "terminusd: config path is too long: %s/%s\n",
                cwd, config_path);
            return EXIT_FAILURE;
        }
    ...
    if (selected_config_path) {
        snprintf(selected_config_path_runtime,
             sizeof(selected_config_path_runtime),
             "%s", selected_config_path);
    ...
    if (test_mode_enabled)
        test_mode_log(LOG_INFO,
                 "[test-mode] Running in test mode: script and shutdown_guard execution are disabled");
    ...
    if (!test_mode_enabled) {
        ...
        test_mode_log(LOG_INFO,
                 "[test-mode] shutdown_guard configured but disabled in "
                 "test mode: command=\"%s\" type=%s",
    ...
    if (control_listen_fd < 0) {
        ...
        test_mode_log(LOG_WARNING,
                 "[test-mode] control socket setup failed; continuing without control socket");
    }
    ...
    if (r < 0) {
        syslog(LOG_ERR,
              "Failed to flush bus after subscription: %s",
              strerror(-r));
        goto finish;
    }
    ...
    for (;;) {
...

        {
            ...
            if (guard_timeout >= 0 &&
                (timeout_ms < 0 || guard_timeout < timeout_ms))
        ...
        if (control_idx >= 0 &&
            ...
            control_handle_socket_ready(control_listen_fd,
