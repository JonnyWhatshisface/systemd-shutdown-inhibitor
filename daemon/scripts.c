...
int apply_entry_credentials(const script_entry_t *e)
{
    ...
    if (have_user && lookup_uid(e->user, &target_uid,
    ...
    if (have_group) {
        ...
        if (!have_primary_gid) {
            fprintf(stderr,
                "[%s] user '%s' requires group "
                "when no passwd entry exists\n",
                e->name, e->user);
            return -1;
        }
    ...
    if (have_user) {
        if (resolved_user[0] != '\0') {
            if (initgroups(resolved_user, target_gid) < 0) {
                fprintf(stderr,
                    "[%s] initgroups(%s) failed: %s\n",
                    e->name, resolved_user,
                    strerror(errno));
                return -1;
            }
    ...
    if (have_group) {
        if (setgid(target_gid) < 0) {
            fprintf(stderr, "[%s] setgid(%lu) failed: %s\n",
                e->name, (unsigned long)target_gid,
                strerror(errno));
            return -1;
        }
    } else if (have_user) {
        if (setgid(primary_gid) < 0) {
            fprintf(stderr, "[%s] setgid(%lu) failed: %s\n",
                e->name, (unsigned long)primary_gid,
                strerror(errno));
            return -1;
        }
    }
    ...
    if (have_user && setuid(target_uid) < 0) {
        fprintf(stderr, "[%s] setuid(%lu) failed: %s\n",
            e->name, (unsigned long)target_uid,
            strerror(errno));
        return -1;
    }
...
char **load_entry_env(const script_entry_t *e)
{
    ...
    if (!f) {
        syslog(LOG_WARNING, "[%s] Failed to open env file %s: %s",
              e->name, e->env, strerror(errno));
        return NULL;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (env_count >= env_capacity - 1) {
            ...
            if (!tmp) {
                syslog(LOG_ERR,
                      "[%s] realloc failed for env array",
                      e->name);
        ...
        if (!env_array[env_count]) {
            syslog(LOG_ERR, "[%s] strdup failed for env entry",
                  e->name);
            for (int i = 0; i < env_count; i++)
                free(env_array[i]);
            free(env_array);
    ...
    syslog(LOG_INFO, "[%s] Loaded %d environment variables from %s",
          e->name, env_count, e->env);
    return env_array;
}
...
{
    if (pipe(out_pipe) < 0) {
        syslog(LOG_ERR, "[%s] pipe() failed: %s",
              e->name, strerror(errno));
        return -1;
    }
...
void run_all_scripts(void)
{
    if (script_count == 0) {
        syslog(LOG_WARNING,
              "PrepareForShutdown received but no scripts configured");
        return;
    }
    ...
    while (i < script_count) {
        ...
        for (int k = 0; k < group_size; k++) {
            ...
            if (e->command[0] == '\0') {
                syslog(LOG_WARNING,
                      "[%s] command not set; skipping",
                      e->name);
                running[k].pid = -1;
                running[k].fd  = -1;
                strncpy(running[k].name, e->name,
                    sizeof(running[k].name) - 1);
                running[k].name[sizeof(running[k].name) - 1] = '\0';
                continue;
            }

            syslog(LOG_INFO,
                "[%s] Launching command=\"%s\" priority=%u user=%s group=%s",
                  e->name, e->command, e->priority,
                ...
                for (ssize_t ci = 0; ci < n; ci++) {
                    ...
                    if (c == '\n' || c == '\r') {
                        ...
                        syslog(LOG_INFO, "[%s] %s",
                              ...
                              running[k].linebuf);
                        running[k].linelen = 0;
                    } else if (running[k].linelen <
                            (int)sizeof(running[k].linebuf) - 1) {
            ...
            if (wret < 0) {
                ...
                if (code == 0)
                    ...
                    if (running[k].critical) {
                        syslog(LOG_ERR,
                              "[%s] critical action failed (exit_code=%d); "
                              "aborting remaining priority groups",
                              running[k].name, running[k].exit_code);
                ...
                if (running[k].critical) {
                    syslog(LOG_ERR,
                          "[%s] critical action failed (signal=%d); "
                          "aborting remaining priority groups",
                          running[k].name, WTERMSIG(status));
                    critical_failed = true;
                }
        ...
        if (critical_failed) {
            syslog(LOG_ERR,
                  "Critical action failure in priority group %u — "
                  "releasing inhibitor lock without running further scripts",
                  cur_prio);
            if (inhibitor_fd >= 0) {
                close(inhibitor_fd);
                inhibitor_fd = -1;
