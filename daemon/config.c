...
static int load_dropin_dir(const char *dir_path)
{
    ...
    if (!dir) {
        ...
        syslog(LOG_WARNING,
              "Failed to open config drop-in directory %s: %s",
    ...
    while ((de = readdir(dir)) != NULL) {
        ...
        if (entry_count >= (int)(sizeof(entries) / sizeof(entries[0]))) {
            syslog(LOG_WARNING,
                  "Too many drop-in files in %s (max %zu); ignoring extras",
                  dir_path, sizeof(entries) / sizeof(entries[0]));
            break;
        }
        ...
        if (!entries[entry_count]) {
            syslog(LOG_ERR,
                  "Out of memory while reading drop-in directory %s",
                  dir_path);
            break;
        }
        entry_count++;
    ...
    for (int i = 0; i < entry_count; i++) {
        ...
        if (n < 0 || (size_t)n >= sizeof(path)) {
            syslog(LOG_WARNING,
                  "Drop-in path too long, skipping %s/%s",
                  dir_path, entries[i]);
            free(entries[i]);
            continue;
        }
        ...
        if (load_config(path) == 0) {
            ...
            syslog(LOG_WARNING,
                  "Failed to load config drop-in %s",
                  path);
        }
...
int load_config(const char *path)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (*p == '[') {
            ...
            if (!close) {
                syslog(LOG_WARNING,
                      "Malformed section header in %s: %s",
                      path, p);
                continue;
            }
            *close = '\0';
            ...
            if (strcmp(section, "main") == 0) {
                ...
                if (!cur) {
                    if (script_count >= MAX_SCRIPTS) {
                        syslog(LOG_WARNING,
                              "Too many script sections (max %d); ignoring [%s]",
                              MAX_SCRIPTS, section);
                        cur = NULL;
                    } else {
        ...
        *eq = '\0';
        ...
        if (strcmp(section, "main") == 0 || section[0] == '\0') {
            if (strcmp(key, "set_max_inhibit_delay") == 0) {
                set_max_inhibit_delay =
                    parse_bool(value, false, key, path);
            } else if (strcmp(key, "restart_logind_after_set") == 0) {
                restart_logind_after_set =
                    parse_bool(value, false, key, path);
                ...
                if (errno == 0 && endptr != value &&
                  *endptr == '\0' && parsed > 0 &&
                  parsed <= INT_MAX) {
                    ...
                    syslog(LOG_WARNING,
                          "Invalid max_inhibit_delay '%s' in %s; using %d",
                          value, path, max_inhibit_delay);
                }
            } else if (strcmp(key, "shutdown_guard_command") == 0) {
                strncpy(guard_config.command, value,
                    sizeof(guard_config.command) - 1);
                ...
                if (strcmp(value, "persist") == 0)
                    ...
                    syslog(LOG_WARNING,
                          "Unknown shutdown_guard_type '%s' in %s; "
                          "using oneshot",
                          value, path);
                ...
                if (errno == 0 && endptr != value &&
                  *endptr == '\0' && parsed > 0)
                    ...
                    syslog(LOG_WARNING,
                          "Invalid shutdown_guard_interval '%s' in %s; "
                          "using %u",
                          value, path, guard_config.interval);
                ...
                if (errno == 0 && endptr != value &&
                  *endptr == '\0' && parsed > 0)
                    ...
                    syslog(LOG_WARNING,
                          "Invalid shutdown_guard_threshold '%s' in %s; "
                          "using %u",
                          value, path, guard_config.threshold);
            } else if (strcmp(key, "shutdown_guard_run_as_user") == 0) {
                strncpy(guard_config.user, value,
                    sizeof(guard_config.user) - 1);
            } else if (strcmp(key, "shutdown_guard_run_as_group") == 0) {
                strncpy(guard_config.group, value,
                    sizeof(guard_config.group) - 1);
            } else if (strcmp(key, "shutdown_guard_run_env") == 0) {
                strncpy(guard_config.env, value,
                    sizeof(guard_config.env) - 1);
            } else if (strcmp(key, "shutdown_guard_enabled") == 0) {
                guard_config.enabled = parse_bool(value, false, key, path);
            } else {
                syslog(LOG_WARNING,
                      "Unknown key '%s' in [main] of %s",
                      key, path);
            }
        } else if (cur) {
            if (strcmp(key, "command") == 0) {
                strncpy(cur->command, value,
                    sizeof(cur->command) - 1);
            } else if (strcmp(key, "user") == 0) {
                strncpy(cur->user, value,
                    sizeof(cur->user) - 1);
            } else if (strcmp(key, "group") == 0) {
                strncpy(cur->group, value,
                    sizeof(cur->group) - 1);
            } else if (strcmp(key, "env") == 0) {
                strncpy(cur->env, value,
                    sizeof(cur->env) - 1);
                ...
                if (errno == 0 && endptr != value &&
                  *endptr == '\0') {
                    ...
                    syslog(LOG_WARNING,
                          "Invalid priority '%s' in [%s] of %s; using %u",
                          value, cur->name, path,
                          cur->priority);
                }
            } else if (strcmp(key, "critical") == 0) {
                cur->critical = parse_bool(value, false,
                                key, path);
            } else if (strcmp(key, "enabled") == 0) {
                cur->enabled = parse_bool(value, true,
                                key, path);
                ...
                if (errno == 0 && endptr != value &&
                  *endptr == '\0' && parsed >= 0 &&
                  parsed <= 255) {
                    ...
                    syslog(LOG_WARNING,
                          "Invalid simulate_exit_code '%s' in [%s] of %s; using 0",
                          value, cur->name, path);
                }
            } else {
                syslog(LOG_WARNING,
                      "Unknown key '%s' in [%s] of %s",
                      key, cur->name, path);
            }
        }
    }
...
/* Remove disabled scripts from the scripts array. */
...
void load_selected_config(const char *config_path)
{
    ...
    if (load_config(path) == 0) {
        syslog(LOG_INFO, "Loaded config from %s (%d script section(s))",
              path, script_count);
        loaded_any = true;
    } else {
        syslog(LOG_WARNING,
    ...
    if (!config_path) {
        ...
        if (dropins > 0) {
            syslog(LOG_INFO,
                  "Loaded %d config drop-in file(s) from %s",
                  dropins, CONF_DROPIN_DIR);
            loaded_any = true;
        }
    }
    ...
    if (!loaded_any) {
        syslog(LOG_WARNING,
              "No usable configuration found; no scripts will run");
    } else {
        syslog(LOG_INFO,
              "Final merged configuration contains %d script section(s)",
              script_count);
    }
}
