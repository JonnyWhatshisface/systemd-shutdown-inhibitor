...
static bool target_is_masked(const char *target)
{
    ...
    if (pipe(pipefd) < 0) {
        syslog(LOG_ERR,
               "[shutdown_guard] pipe() for state probe failed: %s",
               strerror(errno));
        return false;
    }
    ...
    if (pid < 0) {
        syslog(LOG_ERR,
               "[shutdown_guard] fork() for state probe failed: %s",
               strerror(errno));
        close(pipefd[0]);
        close(pipefd[1]);
        return false;
    ...
    if (pid == 0) {
        ...
        if (pipefd[1] > STDERR_FILENO)
            close(pipefd[1]);
        execlp("systemctl", "systemctl", "is-enabled", target,
               (char *)NULL);
        _exit(127);
    }
    ...
    size_t outlen = 0;
    for (;;) {
        ssize_t n = read(pipefd[0], outbuf + outlen,
                         sizeof(outbuf) - 1 - outlen);
        if (n > 0) {
            outlen += (size_t)n;
...
static void sync_mask_state_on_startup(void)
{
    ...
    if (masked_count == 0) {
        syslog(LOG_INFO,
               "[shutdown_guard] startup state sync: shutdown targets currently unmasked");
        return;
    }

    if (masked_count == total) {
        syslog(LOG_INFO,
               "[shutdown_guard] startup state sync: shutdown targets currently masked");
        return;
    }

    syslog(LOG_WARNING,
           "[shutdown_guard] startup state sync: %d/%d shutdown targets are masked; treating guard state as masked",
           masked_count, total);
}
...
static bool apply_mask(bool mask)
{
    ...
    pid_t pid = fork();
    if (pid < 0) {
        syslog(LOG_ERR, "[shutdown_guard] fork() for systemctl: %s",
               strerror(errno));
        free(argv);
    ...
    if (wret < 0) {
        syslog(LOG_ERR, "[shutdown_guard] waitpid(systemctl): %s",
               strerror(errno));
        return false;
    }
    if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
        syslog(LOG_NOTICE, "[shutdown_guard] %s system shutdown and reboot",
               mask ? "DISABLED" : "ENABLED");
        services_masked = mask;
        return true;
    } else {
        syslog(LOG_ERR,
               "[shutdown_guard] systemctl %s --runtime failed (exit %d)",
               mask ? "mask" : "unmask",
...
static pid_t launch_guard_process(int *fd_out)
{
    ...
    if (pid == 0) {
        ...
        snprintf(dummy.user, sizeof(dummy.user),
                 "%s", guard_config.user);
        snprintf(dummy.group, sizeof(dummy.group),
                 "%s", guard_config.group);
        if (apply_entry_credentials(&dummy) < 0)
            _exit(126);

        // load environment
        script_entry_t env_entry = {0};
        snprintf(env_entry.env, sizeof(env_entry.env),
                 "%s", guard_config.env);
        ...
        if (!argv || argc == 0) {
            if (env_is_custom) free_env_array(env);
            _exit(127);
        }
...
static void handle_persist_line(const char *line)
{
    if (strcmp(line, "shutdown_guard_disable_shutdown 1") == 0) {
        syslog(LOG_NOTICE,
               "[shutdown_guard] disabling shutdowns/reboots "
               "(masking shutdown targets)");
        apply_mask(true);
    } else if (strcmp(line, "shutdown_guard_disable_shutdown 0") == 0) {
        syslog(LOG_NOTICE,
               "[shutdown_guard] enabling shutdowns/reboots "
               "(unmasking shutdown targets)");
        apply_mask(false);
    } else {
    ...
    for (;;) {
        ...
        if (n == 0) {
            ...
            if (persist_pid > 0) {
                ...
                syslog(LOG_WARNING,
                       "[shutdown_guard] persist process (pid %d) exited "
                       "(status %d); restarting",
                       (int)persist_pid, code);
                persist_pid = -1;
            }
            ...
            if (persist_pid > 0)
                syslog(LOG_INFO,
                       "[shutdown_guard] persist restarted (pid %d)",
                       (int)persist_pid);
            else
                syslog(LOG_ERR,
                       "[shutdown_guard] failed to restart persist process");
            return;
        }
    ...
    for (;;) {
            ...
            for (ssize_t i = 0; i < n; i++) {
                ...
                if (c == '\n' || c == '\r') {
                    if (oneshot_linelen > 0) {
                        ...
                        syslog(LOG_INFO, "[shutdown_guard] %s",
                               oneshot_linebuf);
                        oneshot_linelen = 0;
                    }
        ...
        if (n == 0) {
            ...
            if (oneshot_pid > 0) {
                ...
                if (wret < 0) {
                    syslog(LOG_ERR, "[shutdown_guard] oneshot waitpid: %s",
                           strerror(errno));
                    fail_count++;
                    ...
                    if (code == 0) {
                        syslog(LOG_INFO,
                               "[shutdown_guard] oneshot OK (exit 0); "
                               "consecutive_failures=0");
                        fail_count = 0;
                        if (services_masked) {
                            syslog(LOG_NOTICE,
                                   "[shutdown_guard] condition cleared; "
                                   "unmasking shutdown targets");
                            apply_mask(false);
                        }
                    } else {
                        fail_count++;
                        syslog(LOG_WARNING,
                               "[shutdown_guard] oneshot exit %d "
                               "(consecutive_failures=%d/%u)",
                               code, fail_count, guard_config.threshold);
                        if ((unsigned int)fail_count >=
                            guard_config.threshold && !services_masked) {
                            syslog(LOG_NOTICE,
                                   "[shutdown_guard] threshold reached; "
                                   "masking shutdown targets");
                            apply_mask(true);
                        }
                    }
                    ...
                    fail_count++;
                    syslog(LOG_WARNING,
                           "[shutdown_guard] oneshot killed by signal %d "
                           "(consecutive_failures=%d/%u)",
                           WTERMSIG(status), fail_count,
        ...
        syslog(LOG_ERR, "[shutdown_guard] oneshot pipe read error: %s",
               strerror(errno));
        close(oneshot_fd);
        oneshot_fd = -1;
        if (oneshot_pid > 0) {
...
void guard_start(void)
{
    ...
    if (guard_config.command[0] == '\0') {
        syslog(LOG_WARNING,
               "[shutdown_guard] enabled but shutdown_guard_command not set; "
               "guard will not run");
        return;
    }
    ...
    if (guard_config.type == GUARD_TYPE_PERSIST) {
        syslog(LOG_INFO,
               "[shutdown_guard] persist mode: command=\"%s\" "
               "user=%s group=%s",
               guard_config.command,
               guard_config.user[0]  ?
                   guard_config.user  : "<daemon>",
               guard_config.group[0] ?
                   guard_config.group : "<daemon>");
        persist_pid = launch_guard_process(&persist_fd);
        if (persist_pid > 0)
            syslog(LOG_INFO,
                   "[shutdown_guard] persistent user-defined shutdown guard started (pid %d)",
                   (int)persist_pid);
        else
            syslog(LOG_ERR,
                   "[shutdown_guard] failed to start persist process");
    } else {
        syslog(LOG_INFO,
               "[shutdown_guard] oneshot mode: command=\"%s\" "
               "interval=%us threshold=%u user=%s group=%s",
               guard_config.command,
               guard_config.interval, guard_config.threshold,
               guard_config.user[0]  ?
                   guard_config.user  : "<daemon>",
               guard_config.group[0] ?
                   guard_config.group : "<daemon>");
...
bool guard_runtime_set_enabled(bool enabled)
{
    ...
    if (guard_config.command[0] == '\0') {
        syslog(LOG_WARNING,
               "[shutdown_guard] terminusctl command requested guard enable, but shutdown_guard_command is empty");
